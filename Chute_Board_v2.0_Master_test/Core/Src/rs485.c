...
\*========================================================================================*/
	extern uint16_t Count_RX_Slave[55];
	extern uint8_t chute_full_arr[55];
	...
	uint8_t RS485_Rx_Handle(rs485_t *pRS)
	{
		if(pRS->RxFlag == 1)
		{
			flag_flashLed = 1;
			//cnt_ping = 0;
			Count_RX_Slave[pRS->rxBuff[2]]++;
			chute_full_arr[pRS->rxBuff[2]] = pRS->rxBuff[3];
			RS485_Reset_Rx(&vRS485);
		}
		return 1;
	}
