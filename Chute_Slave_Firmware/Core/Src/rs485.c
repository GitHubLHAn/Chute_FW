...
\*========================================================================================*/
	...
	void RS485_Init(rs485_t *pRS, UART_HandleTypeDef *huart)
	{
		...
		pRS->ender = RS485_CONFIG_CMD;
		
		pRS->CS_byte = 0;
		pRS->cnt_mess_received = 0;
		pRS->cnt_mess_wrong = 0;
		pRS->cnt_mess_exact = 0;
		...
		memset(pRS->txBuff_config, 0xCB, RS485_NUM_BYTE);
...
//		
		...
		//---------------------------------------------------------------------------------	
			...
			case  RS485_WAIT_STATE:                
			{
				...
				if(pRx->rxByte == 0xEF)
					pRx->STATE = RS485_CHECK_SYNC;
					//count_synchronize = 0;
					
				break;
			} 
		//---------------------------------------------------------------------------------	
			/*Check synchronize denbao*/
			case  RS485_CHECK_SYNC:                
			{
				if(pRx->rxByte == 0xAE)
					count_synchronize = 0;
			...
			case RS485_READING_CONFIG_STATE:
			{
				...
				{    
					...
					if(pRx->rxBuff_config[RS485_NUM_BYTE_CONFIG-1] == pRx->CS_byte
								&& pRx->rxBuff_config[RS485_NUM_BYTE_CONFIG-2] == pRx->ender)
					{ 
						HAL_GPIO_WritePin(RS485_DE_GPIO_Port, RS485_DE_Pin, GPIO_PIN_SET);	
						pRx->RxFlag_config = 1;  
						pRx->cnt_mess_received++;		
...
\*========================================================================================*/
	...
	uint8_t RS485_Rx_Handle(rs485_t *pRS)
	{
		...
		/*Response information to App*/		
			pRS->txBuff[0] = 0xFE;
			pRS->txBuff[1] = ((chute_isFull << 7)&0x80) | 
																((button_Action <<6)&0x40) | 
																					(pRS->Sequence_number & 0x3F);		
			pRS->txBuff[2] = Cal_CheckSum(pRS->txBuff, RS485_NUM_BYTE);
			
		/*Rasie flag send response*/
			pRS->TxFlag = 1;
			HAL_GPIO_WritePin(RS485_DE_GPIO_Port, RS485_DE_Pin, GPIO_PIN_SET);
		...
		/*Handle config commad------------------------------------------------------*/
		if(pRS->RxFlag_config == 1)
		{
			flag_flashLed_trx = true;
			
			for(uint8_t i=0; i< NUM_IDENTIFICATION; i++)
				Get_New_ID_arr[i] = pRS->rxBuff_config[i+1];
			
			Get_New_mode = pRS->rxBuff_config[NUM_IDENTIFICATION+1];
...
\*========================================================================================*/
	...
	uint8_t RS485_Transmit(rs485_t *pRS)
	{
		...
		/*Response normal command*/
		if(pRS->TxFlag == 1)
		{
			HAL_GPIO_WritePin(RS485_DE_GPIO_Port, RS485_DE_Pin, GPIO_PIN_SET);
			interval_wait_send_master = getMicroSecond() - start_wait_send_master;
			if(interval_wait_send_master >= 5000)		// send after 5ms
			{
		...
		if(pRS->TxFlag_config == 1)
		{
			interval_wait_send_master = getMicroSecond() - start_wait_send_master;
			HAL_GPIO_WritePin(RS485_DE_GPIO_Port, RS485_DE_Pin, GPIO_PIN_SET);
			if(interval_wait_send_master >= 2000)	// send after 2 ms
			{
